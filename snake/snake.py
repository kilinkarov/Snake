...
snowflake_image = pygame.transform.scale(snowflake_image, (SIZE, SIZE))
...
def set_difficulty(value, difficulty):
    global difficult, fps, fps_old, fps_water, fps_sand, ffps, count_splitt
    difficult = difficulty
    fps = ffps + difficult * count_splitt
    fps_old = ffps + difficult * count_splitt
    fps_water = fps * 1.5
    fps_sand = fps // 1.5
    pass
...
def table_records():
    ...
    for i in range(min(count_game, 5)):
        ...
        surface_table.blit(render_table, (SIZE, 2 * SIZE + 2 * SIZE * i))
    sleep(5)


def start_the_game():
    global score, count, fps, SIZE, RES, flag, surface,\
        flag_end, flag_snowflake, time, count_game, fps_old, fps_water, fps_sand, snowflake_xx, max_time
    apple = APPLE(x_apple, y_apple, apple_image, lavel[difficult], lavel_sand[difficult],
                  lavel_water[difficult], snowflake_xx, snowflake_xx)
    apple.eating_apple(True, [[RES - SIZE, RES - SIZE]], SIZE)
    snowflake = APPLE(snowflake_xx, snowflake_xx, snowflake_image, lavel[difficult],
                      lavel_sand[difficult], lavel_water[difficult], apple.x, apple.y)
    snake = SNAKE(RES - SIZE, RES - SIZE, lavel[difficult], lavel_sand[difficult],
    ...
    while True:
        ...
        if (snake.snake[-1][0] + SIZE - 1 >= snowflake.x >= snake.snake[-1][0] - SIZE + 1) and (
            ...
            if count == 0:
                ...
                apple.chanche(snowflake_xx, snowflake_xx)
                time = 0
